...
The built-in no-credential retail pass uses [UPCitemdb's free documented API](https://www.upcitemdb.com/api/) for strict brand/model catalog matching. It searches a rotating maximum of 20 unique product groups per day, checks every returned identity, retains merchant links and source-update timestamps, and separates recent offers from older historical catalog observations. Only a recent exact-match merchant offer can show a retail reference and conservative provisional math. Stale offers and historical low/high values remain audit-only; no catalog reference can become used resale evidence, prove demand, or create a safe ceiling.

The included `scripts/enrich-serper-retail.mjs` worker adds a persistent Google Shopping research queue through Serper. Only active, not-yet-ended product listings remain queued; duplicate normalized products share a query, new items are added automatically, and completed or ended items leave the future-work queue without losing their saved research. Never-researched listings closest to closing are checked first. Each completed attempt stores its query, offers, source links, match tier, `checkedAt`/`researchedAt` timestamp, and bounded per-item history. Two independent exact/strong merchants produce a corroborated available reference; one exact/strong merchant is visibly labeled reference-only with the larger uncertainty reserve. Approximate matches remain research-only and can never create a safe ceiling or YES decision. Google Shopping prices are current asking prices, not completed sales or guaranteed resale proceeds.

For broad current-retail coverage, the included Rakuten Advertising adapter searches up to 300 active product groups per hourly/manual pass across every resale vertical. Rakuten's publisher account is free, and its official Product Search API returns merchant, regular price, sale price, UPC, SKU, and a direct product link at up to 100 calls per minute. It requires a publisher bearer token and active retailer partnerships. Results must pass the same strict identity gate, remain labeled new-retail asking prices, and cannot prove used resale value or demand. See [`docs/PRICE_API_SETUP.md`](docs/PRICE_API_SETUP.md).

eBay decommissioned Finding in February 2025, so BidAI Pro no longer calls it. Broad active-used coverage uses the supported [eBay Browse API](https://developer.ebay.com/api-docs/buy/api-browse.html) only when both production client credentials and required eBay approval exist. Close matches remain the preferred eBay tier; broader asking analogs receive a 55%–65% reserve and no safe ceiling. eBay's sales-history [Marketplace Insights API is restricted](https://developer.ebay.com/api-docs/buy/ref-marketplace-supported.html), so the repository keeps its separate authorized completed-sales integration and never fabricates sold data. Paid Google Shopping connectors remain optional.
...
The browser also checks the small `data/snapshot-manifest.json` file once per minute while the page is visible, and again when the tab becomes visible or the browser comes online. It downloads the much larger snapshot only when the manifest revision or check time changes. The hourly GitHub Actions job runs the persistent Serper queue and republishes both files in GitHub's cloud, so price research continues while the computer is off. A GitHub personal-access token is not needed to read, auto-sync, or use **Refresh published data**.

The hourly workflow also runs bounded, failure-isolated public collectors for HiBid, LiveAuctioneers, GovDeals, Public Surplus, PropertyRoom, Proxibid, and BidSpotter using public lot pages or published sitemaps. eBay auction discovery and used-price research use its official Browse API only when the client ID, client secret, and production approval are present. Marketplace cards report **connected**, **collector ready**, **temporarily unavailable**, or **authorization required** from the last real check; a blocked site does not stop other sources or erase retained records. Invaluable remains authorization-only because its published partner API uploads auction catalogs rather than exposing a public listing-read feed.
...
The zero-credential retail-catalog pass is enabled automatically and runs once daily so it stays within the official 20-search/day free allowance. No secret is required. For the persistent Google Shopping queue, add `BIDAI_SERPER_API_KEY` as a GitHub Actions repository secret and optionally set the repository variable `BIDAI_SERPER_MAX_QUERIES_PER_RUN` (default `25`, hard cap `500`). New Serper accounts currently start with 2,500 free queries, but that is an introductory allowance, not a permanently free tier; raising the per-run limit uses it faster. For high-volume current retail, add `BIDAI_RAKUTEN_ACCESS_TOKEN` from a free Rakuten Advertising publisher account; the hourly workflow then checks up to 300 active product groups per pass. To add broad eBay used asking prices, configure both `BIDAI_EBAY_CLIENT_ID` and `BIDAI_EBAY_CLIENT_SECRET` after eBay grants the required production access. Duplicate auction listings share one result, and no credential or access token is written to the repository or public snapshot.

### Independent-price rule

The observed auction bid is an acquisition cost, never a resale comparable. BidAI Pro shows a price only when it can cite independent evidence: matched completed sales, at least five matched used offers, a multi-merchant retail set, a specialty price guide, verified precious-metal inputs, the stored public-web research ledger, an exact retail-catalog reference, or a clearly labeled market analog. Exact retail references are visibly distinguished from used resale estimates and receive a large condition/uncertainty reserve. Broader title/category asking analogs are reference-only, receive a 55%–65% reserve, and cannot create a safe bid ceiling. Every priced item retains source links, observation time, sample size, range, and the reserve applied to that source.

Paid or specialty providers remain optional:

`BIDAI_SERPER_API_KEY` is for the separate persistent Serper queue described above. Serper and SerpApi are different services; `BIDAI_SERPAPI_KEY` does not enable `scripts/enrich-serper-retail.mjs`.
