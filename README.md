...
Resale popularity is separate from auction popularity. When an authorized resale feed supplies a current sold count, active count, and optional median days to sell, BidAI Pro calculates `sell-through = sold / (sold + active)` and a disclosed 0–100 liquidity score. ShopGoodwill bid counts remain labeled auction activity and are never presented as eBay sell-through. When completed sales are unavailable, active used offers from eBay can form a used-price consensus after at least five close title matches survive validation. Those asking prices are labeled separately, receive a configurable 30% haircut before planning calculations, and never masquerade as sold prices or verified sell-through.
