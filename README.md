...
The zero-credential retail-catalog pass is enabled automatically and runs once daily so it stays within the official 20-search/day free allowance. No secret is required. For high-volume current retail, add `BIDAI_RAKUTEN_ACCESS_TOKEN` from a free Rakuten Advertising publisher account; the hourly workflow then checks up to 300 active product groups per pass. To add broad eBay used asking prices, configure both `BIDAI_EBAY_CLIENT_ID` and `BIDAI_EBAY_CLIENT_SECRET` after eBay grants the required production access. Duplicate auction listings share one result, and no credential or access token is written to the repository or public snapshot.
