...
Profit and safe-ceiling calculations expose inbound shipping, tax, buyer premium, marketplace fees, outbound shipping, repair/testing reserve, and return/loss reserve. The safe ceiling is always visible: it is `$0` when no defensible real-price evidence exists. If the source omits inbound shipping, the calculation uses the clearly labeled, user-configurable $25 conservative estimate instead of silently assuming zero. The default queue ranks profitable pawn exits first, then other profitable items by modeled resale popularity and profit. **Closing ≤ 5 min** filters to active auctions ending within five minutes while preserving that profit-and-liquidity ranking. The Learning view uses one fixed-horizon sample per ended listing: the eligible immutable forecast nearest six hours before close, within a three-to-nine-hour window.
