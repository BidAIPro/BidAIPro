...
For additional marketplaces, use **Add authorized feed** on a marketplace card to save an Apify Task ID, Dataset ID, or non-secret HTTPS feed URL as the `BIDAI_SOURCE_CONFIG_JSON` repository variable. A card becomes connected only after real records are successfully ingested; zero-record marketplaces are disabled in the listing filter and explicitly labeled **feed not connected**. The controls do not invent an endpoint or bypass provider authorization. Configure sensitive credentials as GitHub Actions secrets:
