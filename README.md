...
The evidence-weighted queue always orders active records by decision tier: pawn-safe YES, retail-safe YES, evidence-qualified but over-ceiling NO, price-without-demand NO, then insufficient-evidence NO. Within each tier it uses a disclosed 0–100 relative ranking score, projected profit, demand, and research coverage. Dedicated **Highest resale value** and **Most popular** views sort their respective measures from highest to lowest, place missing values last, and keep the evidence type visible on every row. A sub-50 ranking score never overrides a NO. Clicking a row opens its complete dossier in a popup, while the separate source-listing control goes directly to the real auction. **Closing ≤ 5 min** preserves the same ranking inside the five-minute window.
