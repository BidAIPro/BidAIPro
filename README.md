...
The optional broad-market connector supports both [SearchAPI Google Shopping](https://www.searchapi.io/docs/google-shopping) and [SerpApi Google Shopping](https://serpapi.com/shopping-results), which expose merchant, price, condition, rating, review count, and offer links. BidAI Pro first requires closely matched titles, at least five offers, and at least two merchants within the same condition group. Used offers can support resale planning; new offers are a weaker replacement-cost proxy and receive a configurable 45% condition/resale haircut. When close matching fails, a second category search may form a separately labeled analog benchmark with a 55% reserve. Review counts are shown only as product-interest evidence and never converted into sell-through or sale speed.
...
Every item now receives a strict two-gate YES/NO decision. **Gate one: pawn.** A pawn ceiling exists only when fresh precious-metal spot data can be combined with source-stated purity and weight; ordinary used-retail prices and dealer buy guides never become pawn estimates. **Gate two: retail.** A retail price can come from exact-model completed sales, a matched specialty guide, multi-source used offers, or a deeply discounted new-retail proxy—but it receives a bid ceiling only when an independent demand score also passes. Demand can come from sold-versus-active evidence, sufficient recent exact-model completed sales, or reported annual unit volume. Active listings, auction bids, and product reviews cannot pass demand by themselves.

The default retail demand threshold is 55/100 and is adjustable. A retail route that has price evidence but fails demand is shown for research, receives a `$0` retail ceiling, and produces a final **NO — DO NOT BID**. A route that passes demand still must clear all acquisition costs, marketplace fees, shipping, condition reserves, minimum profit, and target margin. The final answer is either **YES — BID UP TO $X** with the suggested channel and proof, or **NO** with the failed pawn, demand, or margin gate.
...
The observed auction bid is an acquisition cost, never a resale comparable. BidAI Pro shows a resale number only when it can cite independent evidence: matched completed sales, at least five matched used offers, a multi-merchant retail set, a specialty price guide, verified precious-metal inputs, the stored public-web research ledger, or a clearly labeled Google Shopping analog. Exact and close matches are preferred. When those fail, at least five real offers from two merchants can create a broader title/category analog with a 55% planning reserve; that number is reference-only and cannot create a safe bid ceiling. Every priced item retains source links, observation time, sample size, range, and the reserve applied to that source.

To add broad retail/used-market coverage and supported collectible price guides, add any applicable secrets:
...
The default workflow processes up to 200 unique Shopping queries per hourly/manual run. Listings with the same normalized query share one API response, so duplicate products do not consume duplicate calls. If the title search cannot produce five offers from two merchants, the collector tries one broader category query. Credentials remain in GitHub Actions secrets and provider tokens are never written to the public data file.
